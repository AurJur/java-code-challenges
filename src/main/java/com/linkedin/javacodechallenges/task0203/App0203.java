...
public class App0203 {
    ...
    public static final Map<Character, Integer> letterPoints = Map.ofEntries(
            ...
            Map.entry('U', 1), Map.entry('W', 4), Map.entry('V', 4), Map.entry('X', 8), Map.entry('Y', 4),
            Map.entry('Z', 10));
    ...
    public static int wordScoreCalculator(String word) {
        ...
        normalizedWord.chars()
                .mapToObj(n -> (char) n)
                .forEachOrdered(letter -> score.getAndAdd(letterPoints.getOrDefault(letter, 0)));
